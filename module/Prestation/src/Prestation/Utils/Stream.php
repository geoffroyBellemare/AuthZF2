...
class Stream
{
    ...
    private function post($string)
    {
        ...
        if (preg_match('/^(.*)\[\]$/i', $match[1], $tmp)) {
            ...
            $data1 = array();
            preg_match('/(\w+)(\[(\w+)\])?(\[(\w+)\])?(\[(\w+)\])?/i', $match[1], $tmp);
            //si la 4 et null a[]null
            $data[$tmp[1]] = empty($tmp[4])?array($tmp[3] => $match[2] ):array($tmp[3] => array($tmp[5] =>array($tmp[7]=>$match[2])));
           // var_dump($data);

            if( !isset($this->tab[$tmp[1]]) )$this->tab[$tmp[1]] = array();
            if( empty($tmp[4]) ) {
                //!!! $this->tab[$tmp[1]][$tmp[3]] = $match[2];
                $this->tab[$tmp[1]] = $match[2];
                ...
                if(!isset($this->tab[$tmp[1]])){

                    $this->tab[$tmp[1]] = array();
                }
                if(!isset($this->tab[$tmp[1]][$tmp[3]])){

                    $this->tab[$tmp[1]][$tmp[3]] = array();
                }
                $this->tab[$tmp[1]][$tmp[3]][$tmp[5]] = $match[2];
            }
